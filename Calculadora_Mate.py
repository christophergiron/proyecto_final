...
def operaciones_matematica_discreta(frame, volver_inicio):
    ...
    prilab = tk.Label(panta_permu, text="Ingrese El Primer Valor: ", font=("Times New Roman", 10)) #coloca el texto que pide introducir los numeros
    ...
    entra_pri = tk.Entry(panta_permu, font=("Times New Roman", 10), width=30) #deja introducir el valor del primer valor
    entra_pri.grid(row=2, column=1, padx=10, pady=10)

    entra_seg = tk.Entry(panta_permu, font=("Times New Roman", 10), width=30) # lo mismo que arriba pero con el segundo valor
    entra_seg.grid(row=3, column=1, padx=10, pady=10)

    rep = tk.Label(panta_permu, text="Tipo:", font=("Times New Roman", 10)) #coloca un texto que dice tipo para saber que repeticion se esta usando
    rep.grid(row=4, column=0, padx=10, pady=10)

    norep = tk.Radiobutton(panta_permu, text="Sin Repeticion", variable=camb_rep, value="Sin Repeticion", font=("Times New Roman", 10)) #es el boton que se muestra, solo que este al ser un radiobutton, es un boton de bolita
    norep.grid(row=4, column=1, padx=10, pady=10)

    conrep = tk.Radiobutton(panta_permu, text="Con Repeticion", variable=camb_rep, value="Con Repeticion", font=("Times New Roman", 10))  # lo mismo que arriba, pero muestra con repeticion en lugar de sin repeticion
    conrep.grid(row=4, column=2, padx=10, pady=10)

    btncalc = tk.Button(panta_permu, text="Calcular", command=calc, font=("Times New Roman", 10), width=50, height=2) # es el boton que llama a la funcion de calcular 
    btncalc.grid(row=6, column=0, columnspan=2, pady=10)

    result = tk.Label(panta_permu, text="", font=("Times New Roman", 10)) #muestra los resultados de las operaciones
    result.grid(row=7, column=1, columnspan=2, pady=10)

    btnpatras_permu = tk.Button(panta_permu, text="Regresar", command=lambda: mostrar(panta_princi), font=("Times New Roman", 10), width=10, height=2) #es el boton que regresa a la pantalla de inicio de la calculadora, el nombre de la extension lo deja muy claro
