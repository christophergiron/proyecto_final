...
def operaciones_matematica_discreta(frame, volver_inicio,):
    ...
    panta_princi = tk.Frame(frame, bg="#498af2")
    panta_permu = tk.Frame(frame, bg="#498af2")
    panta_comb = tk.Frame(frame, bg="#498af2")
    ...
    def calc(): #Define la funcion de calcular
        try:
            ...
            return
    operacionselec = tk.Label(panta_princi, font=("Times New Roman", 10), text="Seleccione su Operación", bg="#498af2") #solo funciona para bajar un poco los botones
    ...
    btnpermu = tk.Button(panta_princi, activebackground="#4c6c9e", bg="#92bcff", text="Permutacion", command=lambda:[mostrar(panta_permu), camb.set("Permutacion")], font=("Times New Roman", 10), width=30, height=3)
    btnpermu.grid(row=1, column=3, padx=302, pady=10)

    btncomb = tk.Button(panta_princi, activebackground="#4c6c9e", bg="#92bcff", text="Combinacion", command=lambda: [mostrar(panta_comb), camb.set("Combinacion")], font=("Times New Roman", 10), width=30, height=3)
    ...
    btn_volver_menu = tk.Button(panta_princi, activebackground="#001a43", bg="#4e2193",text="Volver al Inicio", command=volver_inicio, font=("Times New Roman", 10), width=30, height=3)
    ...
    prilab = tk.Label(panta_permu, bg="#498af2", text="Ingrese El Primer Valor: ", font=("Times New Roman", 10)) #coloca el texto que pide introducir los numeros
    prilab.grid(row=2, column=1, padx=10, pady=10)

    seglab = tk.Label(panta_permu, bg="#498af2", text="Ingrese El segundo valor", font=("Times New Roman", 10))
    seglab.grid(row=3, column=1, padx=10, pady=10)

    entra_pri = tk.Entry(panta_permu, bg="#cde0ff", font=("Times New Roman", 10), width=30) #deja introducir el valor del primer valor
    entra_pri.grid(row=2, column=2, padx=10, pady=10)

    entra_seg = tk.Entry(panta_permu, bg="#cde0ff", font=("Times New Roman", 10), width=30) # lo mismo que arriba pero con el segundo valor
    entra_seg.grid(row=3, column=2, padx=10, pady=10)

    rep = tk.Label(panta_permu, bg="#498af2", text="Tipo:", font=("Times New Roman", 10)) #coloca un texto que dice tipo para saber que repeticion se esta usando
    rep.grid(row=4, column=1, padx=2, pady=10)

    norep = tk.Radiobutton(panta_permu, activebackground="#4c6c9e", bg="#92bcff",text="Sin Repeticion", variable=camb_rep, value="Sin Repeticion", font=("Times New Roman", 10)) #es el boton que se muestra, solo que este al ser un radiobutton, es un boton de bolita
    norep.grid(row=4, column=2,  padx=0, pady=10)

    conrep = tk.Radiobutton(panta_permu, activebackground="#4c6c9e", bg="#92bcff", text="Con Repeticion", variable=camb_rep, value="Con Repeticion", font=("Times New Roman", 10))  # lo mismo que arriba, pero muestra con repeticion en lugar de sin repeticion
    conrep.grid(row=4, column=2, columnspan=20, padx=0, pady=10)

    btncalc = tk.Button(panta_permu, activebackground="#4c6c9e", bg="#92bcff", text="Calcular", command=calc, font=("Times New Roman", 10), width=50, height=2) # es el boton que llama a la funcion de calcular 
    btncalc.grid(row=6, column=1, columnspan=2, pady=10)

    result = tk.Label(panta_permu,  bg="#cde0ff", text="", font=("Times New Roman", 10,)) #muestra los resultados de las operaciones
    result.grid(row=7, column=1, columnspan=2, pady=10)

    btnpatras_permu = tk.Button(panta_permu, activebackground="#001a43", bg="#4e2193", text="Regresar", command=lambda: mostrar(panta_princi), font=("Times New Roman", 10), width=10, height=2) #es el boton que regresa a la pantalla de inicio de la calculadora, el nombre de la extension lo deja muy claro
    ...
    prilabc = tk.Label(panta_comb, bg="#498af2", text="Ingrese El Primer Valor: ", font=("Times New Roman", 10))
    prilabc.grid(row=2, column=1, padx=10, pady=10)

    seglabc = tk.Label(panta_comb, bg="#498af2", text="Ingrese El segundo valor", font=("Times New Roman", 10))
    seglabc.grid(row=3, column=1, padx=10, pady=10)

    entra_pric = tk.Entry(panta_comb, bg="#cde0ff", font=("Times New Roman", 10), width=30)
    entra_pric.grid(row=2, column=2, padx=10, pady=10)

    entra_segc = tk.Entry(panta_comb, bg="#cde0ff", font=("Times New Roman", 10), width=30)
    entra_segc.grid(row=3, column=2, padx=10, pady=10)

    repc = tk.Label(panta_comb, bg="#498af2", text="Tipo:", font=("Times New Roman", 10))
    repc.grid(row=4, column=1, padx=10, pady=10)

    norepc = tk.Radiobutton(panta_comb, activebackground="#4c6c9e", bg="#92bcff", text="Sin Repeticion", variable=camb_rep, value="Sin Repeticion", font=("Times New Roman", 10))
    norepc.grid(row=4, column=2, padx=0, pady=10)

    conrepc = tk.Radiobutton(panta_comb, activebackground="#4c6c9e", bg="#92bcff", text="Con Repeticion", variable=camb_rep, value="Con Repeticion", font=("Times New Roman", 10))
    conrepc.grid(row=4, column=2, columnspan=20,padx=10, pady=10)

    btncalcc = tk.Button(panta_comb, activebackground="#4c6c9e", bg="#92bcff", text="Calcular", command=calc, font=("Times New Roman", 10), width=50, height=2)
    btncalcc.grid(row=6, column=1, columnspan=2, pady=10)

    resultc = tk.Label(panta_comb,  bg="#cde0ff", text="", font=("Times New Roman", 10))
    resultc.grid(row=7, column=1, columnspan=2, pady=10)

    btnpatras_com = tk.Button(panta_comb, activebackground="#001a43", bg="#4e2193", text="Regresar", command=lambda: mostrar(panta_princi), font=("Times New Roman", 10), width=10, height=2)
